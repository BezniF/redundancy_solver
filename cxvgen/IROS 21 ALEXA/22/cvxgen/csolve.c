...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "dotq_adm");
  if (xm == NULL) {
    printf("could not find params.dotq_adm.\n");
  } else {
    if (!((mxGetM(xm) == 6) && (mxGetN(xm) == 1))) {
      printf("dotq_adm must be size (6,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter dotq_adm must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter dotq_adm must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter dotq_adm must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.dotq_adm;
      src = mxGetPr(xm);
      for (i = 0; i < 6; i++)
        *dest++ = *src++;
      valid_vars++;
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("dotq_max must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("dotq_min must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "h_safe_2");
  if (xm == NULL) {
    printf("could not find params.h_safe_2.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("h_safe_2 must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter h_safe_2 must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter h_safe_2 must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter h_safe_2 must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.h_safe_2;
      src = mxGetPr(xm);
      for (i = 0; i < 1; i++)
        *dest++ = *src++;
      valid_vars++;
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "sigma_obs2");
  if (xm == NULL) {
    printf("could not find params.sigma_obs2.\n");
  } else {
    if (!((mxGetM(xm) == 6) && (mxGetN(xm) == 1))) {
      printf("sigma_obs2 must be size (6,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter sigma_obs2 must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter sigma_obs2 must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter sigma_obs2 must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.sigma_obs2;
      src = mxGetPr(xm);
      for (i = 0; i < 6; i++)
        *dest++ = *src++;
      valid_vars++;
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "theta");
  if (xm == NULL) {
    printf("could not find params.theta.\n");
  } else {
    if (!((mxGetM(xm) == 2) && (mxGetN(xm) == 1))) {
      printf("theta must be size (2,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter theta must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter theta must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter theta must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.theta;
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 6; i++)
      printf("  params.sigma_obs2[%d] = %.6g;\n", i, params.sigma_obs2[i]);
    for (i = 0; i < 1; i++)
      printf("  params.h_safe_2[%d] = %.6g;\n", i, params.h_safe_2[i]);
    for (i = 0; i < 1; i++)
